...
class RegisterForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'username': widgets.TextInput(attrs={"class":"login_txtbx", "placeholder": "请输入用户名"}),
            'mobile': widgets.TextInput(attrs={"class": "login_txtbx", "placeholder": "请输入手机号"}),
            'password': widgets.PasswordInput(attrs={"class": "login_txtbx", "placeholder": "请输入密码"}),
        }
    ...
    def clean_password(self):
        ...
        if len(data)<8:
            raise ValidationError("密码长度小于8，请重新输入")
        else:
            if not data.isdigit():
...
class LoginForm(forms.Form):
    username = forms.CharField(label="用户名", max_length="24",
                               widget=widgets.TextInput(attrs={"class": "login_txtbx", "placeholder": "用户名"}))
    captcha = forms.CharField(label="验证码", widget=widgets.TextInput(
        attrs={"class":"login_txtbx" ,
               "placeholder": "验证码",
               "onblur": "check_captcha()",
               "style": "width:100%",
               "error_messages": {"invalid": "验证码错误"}}))
    password = forms.CharField(label="密 码",
                               widget=widgets.PasswordInput(attrs={"class": "login_txtbx", "placeholder": "请输入密码"}))
